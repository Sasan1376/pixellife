(() => {
  ...
  const start = () => {
    ...
    toggle.innerHTML = '<svg viewBox="0 0 32 32" aria-hidden="true"><path d="M16 5.2a10.8 10.8 0 1 0 10.8 10.8" fill="none" stroke="currentColor" stroke-width="2.7" stroke-linecap="round"/><path d="M16 9a7 7 0 1 0 7 7" fill="none" stroke="currentColor" stroke-width="2.7" stroke-linecap="round" opacity=".92"/><circle cx="16" cy="16" r="2.6" fill="currentColor"/><path d="m25.2 5 .75 2.05L28 7.8l-2.05.75-.75 2.05-.75-2.05-2.05-.75 2.05-.75z" fill="currentColor"/></svg>';
    const panel = add("section", { id: "plAssistant", ariaLabel: "دستیار خرید PixelLife" });
    panel.innerHTML = '<div class="pl-ai-head"><div class="pl-ai-title"><span>دستیار خرید PixelLife</span><small>قدرت گرفته از OpenAI</small></div><button id="plAssistantClose" type="button" aria-label="بستن">×</button></div><div class="pl-ai-messages" id="plAssistantMessages"><div class="pl-ai-msg bot">سلام! برای انتخاب گوشی، تبلت یا کنسول مناسب راهنمایی‌تان می‌کنم. بودجه و کاربردتان را بنویسید.</div></div><form class="pl-ai-form" id="plAssistantForm"><input id="plAssistantInput" maxlength="800" autocomplete="off" placeholder="مثلاً گوشی مناسب بازی تا ۵۰ میلیون" /><button type="submit">ارسال</button></form>';
